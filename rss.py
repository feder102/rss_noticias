...
# Obtén el token de acceso de tu bot y el ID de chat de tu canal de Telegram
TOKEN = '****'
CHAT_ID = '***'
...
def send_rss_feed():
    ...
    # Obtener el día, el mes y la fecha
    ...
    for entry in feed.entries[:20]:
        ...
        if(hour == 20):
            # Envía el título y el enlace del art?culo a trav?s del bot de Telegram
            if (diferencia_dias.days == 0):
                if(fecha_hora.hour <= 20 and fecha_hora.hour >= 14):
                    bot.send_message(
                        chat_id=CHAT_ID, text=f"{entry.title}\n{entry.link}")
        elif(hour == 14):
            # Envía el t?tulo y el enlace del art?culo a trav?s del bot de Telegram
            if (diferencia_dias.days == 0):
...
# Llama a la función para enviar el feed de RSS
send_rss_feed()
